...
export default function CancelledProjects(): JSX.Element {
    const { isAuthenticated, user } = useContext(AuthContext);
    const [project, setProjects] = useState<CancelledProject[]>([]);
        ...
        async function getProjects() {
            try {
                ...
                const { data } = await api.get(
                    `/cancelledProjects/${portfolioId}`
                );

                if (data.length < 1) {
                    setProjects([]);
                    toast.error('Nenhum projeto foi cancelado');
                    return;
                }
