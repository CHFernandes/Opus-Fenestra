...
class OrganizationsController {
    async create(request: Request, response: Response): Promise<Response> {
        const { name, mission, values, vision } = request.body;

        try {
            const organization = await singletonOrganizations
                .getInstance()
                .create(name, mission, values, vision);
    ...
    async show(request: Request, response: Response): Promise<Response> {
        try {
            const organizationsList = await singletonOrganizations
                .getInstance()
                .list();
    ...
    async showById(request: Request, response: Response): Promise<Response> {
        ...
        try {
            const list = await singletonOrganizations
                .getInstance()
                .findById(Number(id));
