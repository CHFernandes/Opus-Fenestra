...
class ProjectsStatusController {
    async lastChanged(request: Request, response: Response): Promise<Response>{
        ...
        try {
            const project = await singletonProject.getInstance().findLastProjectChangedStatus(Number(id));
    ...
    async lastChangedById(request: Request, response: Response): Promise<Response>{
        const {id} = request.params;

        try {
            const project = await singletonProject.getInstance().findLastProjectChangedStatusByProject(Number(id));
