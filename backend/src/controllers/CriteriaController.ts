...
class CriteriaController {
    async create(request: Request, response: Response): Promise<Response> {
        const { idPortfolio, description, weight, idUnities } = request.body;

        try {
            const criterion = await singletonCriteria
                .getInstance()
                .create(
                    description,
                    weight,
    ...
    async show(request: Request, response: Response): Promise<Response> {
        ...
        try {
            const criteriaList = await singletonCriteria
                .getInstance()
                .list(Number(id));
    ...
    async showById(request: Request, response: Response): Promise<Response> {
        const { id } = request.params;
        try {
            const list = await singletonCriteria
                .getInstance()
                .findById(Number(id));
    ...
    async updateById(request: Request, response: Response): Promise<Response> {
        const { id } = request.params;
        const { description, weight, idPortfolio, idUnities } = request.body;

        try {
            const updatedCriterion = await singletonCriteria
                .getInstance()
                .updateById(
                    description,
                    weight,
    ...
    async deleteById(request: Request, response: Response): Promise<Response> {
        const { id } = request.params;
        try {
            const responseDelete = await singletonCriteria
                .getInstance()
