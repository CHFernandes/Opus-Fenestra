...
class ProjectsStatusService {
    ...
    async findLastProjectChangedStatus(
        id_portfolio: number
        ...
        const list = await getConnection()
            ...
            .leftJoin(
                Person,
                'person',
                'person.id_person = project_status.id_person'
            )
            .leftJoin(
                Project,
                'project',
                'project.id_project = project_status.id_project'
            )
            .leftJoin(
                Status,
                'status',
                'status.id_status = project_status.id_status'
            )
    ...
    async findLastProjectChangedStatusByProject(
        id_project: number
    ): Promise<ProjectStatus[]> {
        if (!id_project) {
            throw new Error('Campos obrigatórios não preenchidos');
